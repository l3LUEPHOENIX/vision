...
argParser.add_argument(
    "-d", "--directory", help="The directory to watch", required=True
)
argParser.add_argument("-u", "--url", help="URL to Vision", required=True)
argParser.add_argument(
    "-k", "--apikey", help="Api Key to authenticate with Vision", required=True
)
...
def send_post_request(file_path, file_name, action_type):
    ...
    if response.status_code == 200:
        print(
            f"File {action_type}: {file_name} ({file_size} bytes) - POST request sent"
        )
    else:
        print(
            f"File {action_type}: {file_name} ({file_size} bytes) - POST request failed"
        )
